					...
					<% String role=(String) session.getAttribute("role"); if ("ADMIN".equals(role)) { %>
						...
						<div class="sidebar role-admin" id="adminSidebar">
							...
							<span class="nav-label">Overview</span> <a
								...
								Upload Report
							</a> <a href="<%=request.getContextPath()%>/viewAllReports" data-admin-link="reports"> <i
						...
						<script>
							...
							(function () {
								...
								let activeLink = null;
								if (pageAttr) {
									activeLink = links.find(link => ((link.getAttribute('data-admin-link') || '')
										.toLowerCase() === pageAttr));
								}

								if (!activeLink) {
									activeLink = links.find(link => {
										const linkHref = (link.getAttribute('href') || '').toLowerCase();
										return linkHref && currentPath.endsWith(linkHref) || (linkHref && currentPath.includes(linkHref));
									});
								}
						...
						<% } else { response.sendRedirect(request.getContextPath() + "/Pages/unauthorizedUser.jsp" );
							return; } %>
			</body>
